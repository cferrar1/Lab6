...
double mostIsolated(vector<double> & number)
{
	...
	double maxdiff = 0;
	double currdiff = 0;
	double cdiffup, cdiffdown;
	for (int i = 0; i < number.size(); i++)
	{
		if (i == 0)
		{
			currdiff = abs(number.at(i + 1) - number.at(i));
		}
		else if (i == number.size() - 1)
		{
			currdiff = abs(number.at(i) - number.at(i-1));
		}
		else
		{
			cdiffdown = abs(number.at(i) - number.at(i - 1));
			cdiffup = abs(number.at(i + 1) - number.at(i));
			if (cdiffdown <= cdiffup)
			{
				currdiff = cdiffdown;
			}
			else
